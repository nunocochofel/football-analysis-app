...
// Candidato C2 (diagnóstico do crash de arranque no macOS): sql.js instancia um módulo WASM
// grande — initDatabase() deixou de correr no boot (app.whenReady(), antes de qualquer janela
// existir) e passa a correr só quando a primeira query da BD é mesmo chamada. initDatabase() é
// idempotente (db/index.ts guarda o singleton em `db`), por isso este proxy só paga o custo do
// WASM uma vez, na primeira chamada, e as seguintes passam direto.
const q = new Proxy(dbQueries, {
  get(target, prop) {
