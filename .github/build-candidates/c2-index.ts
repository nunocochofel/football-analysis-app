...
// Candidato C2 (diagnóstico do crash de arranque no macOS): já não chama initDatabase() aqui —
// db/index.ts só instancia o WASM do sql.js quando a primeira query da BD é mesmo chamada (ver o
// proxy em ipc.ts). registerIpcHandlers() continua a correr no arranque (é só registo de
// handlers, não toca no sql.js).
