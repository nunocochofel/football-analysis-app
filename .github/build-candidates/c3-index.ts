...
// Candidato C3 (diagnóstico do crash de arranque no macOS): a janela é criada logo a seguir a
// app.whenReady(), antes de qualquer IPC ou base de dados — createWindow() já não está dentro do
// try/catch de initDatabase()/registerIpcHandlers(). O registo de IPC e initDatabase() só correm
// depois de a página terminar de carregar ('did-finish-load'), isolando se algo nessa
// inicialização (não a própria criação da janela/renderer) é o que provoca o crash. Um erro aqui
// já não fecha a app (a janela já está visível) — fica só registado/mostrado; features que
// dependem de IPC/DB não vão funcionar nesse caso, mas a app não desaparece sem explicação.
app.whenReady().then(() => {
  createWindow()
